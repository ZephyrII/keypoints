...
from config import DATASET_PATH, NUM_CLASSES, IMG_HEIGHT, IMG_WIDTH, RADIUS, epochs, batch_size
from src.model import Keypoints
from src.charger_dataset import ChargerDataset, transform
...
def fit(train_data, test_data, model, loss_function, epochs, checkpoint_path = ''):
    for epoch in range(epochs):
        ...
        for i_batch, sample_batched in enumerate(train_data):
            ...
            print('[%d, %5d] loss: %.3f' % (epoch + 1, i_batch + 1, loss.data), end='')
            print('\r', end='')
...
# dataset
train_dataset = ChargerDataset(DATASET_PATH,
...
test_dataset = ChargerDataset(opj(DATASET_PATH, "val"),
                           NUM_CLASSES, IMG_HEIGHT, IMG_WIDTH, RADIUS, transform=transform)
